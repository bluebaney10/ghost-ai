...
3. When dragging a shape, include the nape name and default size in drag payload.
...
5. On drop:
   ...
   - convert the screen position to canvas coordinated using React Flow
...
- Drop logic create new canvas nodes with the expected shape data.
- New nodes use the custom canvas node type.
- `npm run build` passed without types errors.
